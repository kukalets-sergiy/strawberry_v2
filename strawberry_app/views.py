...
def search(request):
    ...
    if query:
        cultures = Culture.objects.filter(Q(culture__icontains=query))

        months = Months.objects.filter(
            Q(month__icontains=query)
            | Q(description__icontains=query)
            | Q(culture__culture__icontains=query)
        )
    else:
...
def month_list(request):
    ...
    context = {"months": serializer.data, "search_query": query}
...
def upload_images(request):
    ...
    return render(
        request=request, template_name="upload_images.html", context={"form": form, "plants": plants}
    )
...
def choose_month(request, month_id=None):
    if request.method == "GET":
        ...
        if strawberry:
            ...
            return render(
                request, "choose_month.html", {"months": months, "selected_month": selected_month}
            )
...
def choose_culture(request, month_id=None):
    if request.method == "GET":
        ...
        return render(
            request,
            "choose_culture.html",
            {"cultures": cultures, "selected_culture": selected_culture, "months": months},
        )
